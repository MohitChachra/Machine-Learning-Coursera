...
delta2 = delta2(:,2:end);   % 5000 x 25  % removing bias unit 

% we donot count for delta 1 because it is input we do not have error in input
...
J_reg = (lambda/(2*m)) * (sum(sum(Theta1(:,2:end).*Theta1(:,2:end))) + sum(sum(Theta2(:,2:end).*Theta2(:,2:end))));
